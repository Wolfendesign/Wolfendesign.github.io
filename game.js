	...
	if (score[prevTurn] >= gridValue) {
		...
		for (i = 0; i < classes.length; i += 1) {
			...
			if (classes[i].indexOf('row') !== -1 || classes[i].indexOf('col') !== -1 || classes[i].indexOf('dia') !== -1) {
				...
				// As mentioned previously, this returns true to fnchoose.
				if (cnt == gridValue) {
					return true;
				}
			}
...
function fnNewGame() {
	...
	// Grid size is increased according to the option chosen by the player.
...
function isEven(value) {
	// Just checks for an even number.
	return value % 2 == 0;
}